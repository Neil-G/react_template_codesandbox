                    ...
                    <tbody>
                        {
                            Object.keys(updaterMethods).map(methodName => {
                                ...
                                const isGraphql = get(instructions, 'serviceOptions.url', '').includes('graphql')
                                return <tr>
                                    <td>{methodName}</td>
                                    <td>{instructions.updateType} {isGraphql && '[graphql]'}</td>
                                    <td>
                                        {/* {
                                            instructions.args && 
                                        ...
                                        {   instructions.args &&
                                            Object.keys(instructions.args).map(key => {
                                                return <div>{key} [{typeof instructions.args[key]}]</div>
                                            })
                                        }
                                    </td>
                                    <td>
                                        <ul>
                                            {(instructions.successActions || []).map(action => {
                                                return <li>
                                                {action.name || action.description} [{action.type}] {action.uiEventFunction && `[ui event]`}
                                                </li>
                                            })}
                                        </ul>
                                    </td>
                    ...
                    <tbody>
                        {
                            pages.map(({ path, component }) => {
                                const pageComponentName = get(component, 'WrappedComponent.name') ||  get(component, 'name')
                                return <tr>
                                    <td>{path}</td>
